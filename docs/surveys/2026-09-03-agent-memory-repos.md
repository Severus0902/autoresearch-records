...
2. `MemoryAgentBench`：看 incremental multi-turn interaction 的 schema，决定是否复用。
3. `LoCoMo`：作为长期对话 memory 的经典支撑和对照。
4. `PersonaMem`：抽取 personalization/dynamic profile 的任务模式。
5. `Mem0` / `Letta` / `MIRIX`：作为 memory backend baseline，而不是一开始就改它们的源码。
